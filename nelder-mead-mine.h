...
class Searcher {
    ...
    public:
        ...
        Searcher(
            ...
            const std::vector<T> &_maximum_limits = {},
            bool _adaptive = true
            ) :
            adaptive(_adaptive),
            tol_fun(1e-8),
            ...
            max_iter(1000),
            max_fun_evals(1000),
            minimum_limits(_minimum_limits),
            maximum_limits(_maximum_limits),
            _converged(false),
            verbose(false) {
            initialize(initial_point);
        }
        void function_tolerance(T tol) { tol_fun = tol; }
        void point_tolerance(T tol) { tol_x = tol; }
        void initialize(const std::vector<T> &initial_point) {
            ...
            for (unsigned int i = 1; i <= dimension; i++) {
                ...
                p(i - 1) += tau;
                // enforce limits!
                if (minimum_limits.size() == dimension) p.enforce_min(minimum_limits);
                if (maximum_limits.size() == dimension) p.enforce_max(maximum_limits);
                simplex[i] = p;
            }
        ...
        const std::vector<T>& get_next_point(void) {
            ...
            switch(stage) {
                ...
                case REFLECTION: {
                    ...
                    // enforce limits!
                    if (minimum_limits.size() == dimension) x_r.enforce_min(minimum_limits);
                    if (maximum_limits.size() == dimension) x_r.enforce_max(maximum_limits);
                ...
                case EXPANSION: {
                    ...
                    // enforce limits!
                    if (minimum_limits.size() == dimension) x_e.enforce_min(minimum_limits);
                    if (maximum_limits.size() == dimension) x_e.enforce_max(maximum_limits);
        ...
        void check_for_convergence(void) {
            ...
            if ((max_val_diff <= tol_fun and max_point_diff <= tol_x) or
                ...
                std::cout << "Converged after " << niter << " iterations." << std::endl;
        ...
        void evaluate_reflection(void) {
            ...
            // reflection fell through, so stay in reflection stage - but get a new point
            stage = REFLECTION;
        }
