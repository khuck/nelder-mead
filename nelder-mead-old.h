...
std::vector<T> find_min(const Function &func, std::vector<T> &initial_point,
    ...
    while (max_iter--) {
        ...
        if (not value_cache[0].first) {
            val = f(simplex[0]);
            // std::cout << "first 0 " << simplex[0].to_string() << " = " << val
            // << std::endl;
        ...
        // the first point in the simplex is our baseline, so set things up
        // accoringly
        biggest_val        = val;
        smallest_val       = val;
        ...
        for (unsigned int i = 1; i < simplex.size(); i++) {
            ...
            if (not value_cache[i].first) {
                val = f(simplex[i]);
                // std::cout << "first " << i << " " << simplex[i].to_string()
                // << " = " << val << std::endl;
        ...
        if ((max_val_diff <= tol_fun and max_point_diff <= tol_x) or
            ...
            std::cout << "Converged after " << niter - max_iter
                      << " iterations." << std::endl;
        ...
        // enforce limits!
        if (minimum_limit.size() == dimension)
            x_r.enforce_min(minimum_limit);
        if (minimum_limit.size() == dimension)
            x_r.enforce_max(maximum_limit);
        ...
        if (reflection_val < smallest_val) {
            ...
            // enforce limits!
            if (minimum_limit.size() == dimension)
                x_e.enforce_min(minimum_limit);
            if (minimum_limit.size() == dimension)
                x_e.enforce_max(minimum_limit);
            ...
            if (expansion_val < reflection_val) {
                ...
                // otherwise, replace our worst simplex with our reflection
                // point
            ...
            //  is the contraction better than the reflection or the known
            //  worst?
